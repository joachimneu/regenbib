...
class TestRender:
    ...
    def test_biblatex_group_collapses_duplicate_content(self, workdir, monkeypatch, stub_lookups):
        Store([DblpEntry("key-a", "conf/x/Y99"), DblpEntry("key-b", "conf/x/Y99")]).dump(
            str(workdir / "dup.yaml")
        )
        out = workdir / "out.bib"
        _argv(
            monkeypatch,
    ...
    @pytest.mark.parametrize(
        "flag", ["--delay-dblp", "--delay-arxiv", "--delay-eprint", "--delay-doi"]
    )
...
class TestImportAuxParsing:
    ...
    def test_extracts_both_citation_forms_in_order(self, workdir, aux_file, monkeypatch, capsys):
    ...
    def test_entries_already_present_are_skipped(self, workdir, aux_file, monkeypatch, capsys):
        monkeypatch.setattr(cli_import, "attempt_import", lambda methods: None)
        _argv(
            monkeypatch,
            "--aux",
    ...
    def test_yaml_untouched_when_nothing_to_import(self, workdir, aux_file, monkeypatch):
        target = workdir / "references.yaml"
        before = target.read_text()
        monkeypatch.setattr(cli_import, "attempt_import", lambda methods: None)
        _argv(
    ...
    def test_imported_entry_is_appended_and_persisted(self, workdir, aux_file, monkeypatch):
